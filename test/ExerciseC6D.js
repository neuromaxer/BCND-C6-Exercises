...
contract("ExerciseC6D", async (accounts) => {
    ...
    before("setup contract", async () => {
        ...
        const ON_TIME = 10;
        // config.exerciseC6D.events.allEvents(
        //     { fromBlock: 1 },
        //     (error, result) => {
        //         console.log("IM HERE");
        ...
        //                     result.args.status.toNumber() == ON_TIME
        //                         ? "ON TIME"
        //                         : "DELAYED"
        //                 }, verified: ${
        //                     result.args.verified ? "VERIFIED" : "UNVERIFIED"
        //                 }`
        //             );
        //         }
        //     }
    ...
    it("can register oracles", async () => {
        ...
        for (let a = 1; a < TEST_ORACLES_COUNT; a++) {
            ...
            console.log(
                `Oracle Registered: #${a}: ${result[0]}, ${result[1]}, ${result[2]}`
            );
        }
    });
    ...
    it("can request flight status", async () => {
        ...
        resFlightStatus = await config.exerciseC6D.fetchFlightStatus(
            flight,
            timestamp
        );
        console.log(
            "Flight Status: ",
            resFlightStatus.logs[0]["args"]["index"].toNumber()
        );
        // ACT
        ...
        for (let a = 1; a < TEST_ORACLES_COUNT; a++) {
            ...
            for (let idx = 0; idx < 3; idx++) {
                try {
                    ...
                    res = await config.exerciseC6D.submitOracleResponse(
                        oracleIndexes[idx],
                        flight,
                        timestamp,
                        10,
